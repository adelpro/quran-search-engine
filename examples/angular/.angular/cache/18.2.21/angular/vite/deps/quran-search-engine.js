...
// ../../dist/index.mjs
var loadMorphology = () => __async(void 0, null, function* () {
  try {
    const morphologyModule = yield import("./morphology-QCASLOZB-SKYVJWXW.js");
    const morphologyArray = morphologyModule.default || morphologyModule;
    const morphologyMap = /* @__PURE__ */ new Map();
...
var loadWordMap = () => __async(void 0, null, function* () {
  try {
    const wordMapModule = yield import("./word-map-7GKBHIQA-W2V7ULD4.js");
    return wordMapModule.default || wordMapModule;
  } catch (error) {
...
var loadQuranData = () => __async(void 0, null, function* () {
  try {
    const quranModule = yield import("./quran-IEIJ6Y6C-BEUGOST3.js");
    return quranModule.default || quranModule;
  } catch (error) {
    console.error("Failed to load Quran data:", error);
    throw new Error("Could not load Quran data. Ensure src/data/quran.json exists.");
  }
});
var removeTashkeel = (text) => {
  return text.replace(/\u0671/g, "ا").replace(/[\u064B-\u065F\u0670\u06D6-\u06DC\u06DF-\u06E8\u06EA-\u06FC]/g, "");
};
var normalizeArabic = (text) => {
  if (!text) return "";
...
var createDiacriticRegex = (token) => {
...
var getHighlightRanges = (text, matchedTokens, tokenTypes) => {
  ...
  return finalRanges.map((_a) => {
    var _b = _a, {
      priority: _priority
    } = _b, range = __objRest(_b, [
      "priority"
    ]);
    return range;
  });
};
var getPositiveTokens = (verse, mode, targetLemma, targetRoot, cleanQuery, morphologyMap, wordMap) => {
  if (!cleanQuery) return [];
  const normalizedQuery = normalizeArabic(cleanQuery);
  if (!normalizedQuery) return [];
  if (mode === "text") {
    const words = (verse.standard || "").split(/\s+/).map((w) => w.replace(/[^\u0621-\u064A]/g, ""));
...
var createArabicFuseSearch = (collection, keys, options = {}) => new Fuse(collection, __spreadValues({
  includeScore: true,
  includeMatches: true,
  threshold: 0.5,
  distance: 100,
...
var simpleSearch = (items, query, searchField) => {
...
var computeScore2 = (verse, cleanQuery, morphologyMap, wordMap, options, mapEntry, fuseMatches) => {
  let score = 0;
  let matchType = "none";
  let matchedTokens = [];
  const tokenTypes = {};
  ...
  for (const token of queryTokens) {
    const textMatches = getPositiveTokens(verse, "text", void 0, void 0, token, morphologyMap);
    ...
    if (entry) {
      if (options.lemma && entry.lemma) {
        const lemmaMatches = getPositiveTokens(verse, "lemma", entry.lemma, void 0, token, morphologyMap);
      ...
      if (options.root && entry.root) {
        const rootMatches = getPositiveTokens(verse, "root", void 0, entry.root, token, morphologyMap, wordMap);
  ...
  return __spreadProps(__spreadValues({}, verse), {
    matchScore: score,
    matchType,
    matchedTokens,
    tokenTypes
...
var performAdvancedLinguisticSearch = (query, quranData, options, fuseInstance, wordMap, morphologyMap) => {
  const cleanQuery = normalizeArabic(query.replace(/[^\u0600-\u06FF\s]+/g, "").trim());
  if (!cleanQuery) return [];
  const tokens = cleanQuery.split(/\s+/);
  const tokenMatches = tokens.map((token) => {
    const entry = wordMap[token];
    const matchingGids = /* @__PURE__ */ new Set();
    if (entry) {
      const {
        lemma: targetLemma,
        root: targetRoot
      ...
      if (options.lemma && targetLemma) {
        for (const verse of quranData) {
          const morph = morphologyMap.get(verse.gid);
          if (morph?.lemmas.some((lemma) => normalizeArabic(lemma).includes(normalizeArabic(targetLemma)))) {
      ...
      if (options.root && targetRoot) {
        for (const verse of quranData) {
          const morph = morphologyMap.get(verse.gid);
          if (morph?.roots.some((root) => normalizeArabic(root).includes(normalizeArabic(targetRoot)))) {
            matchingGids.add(verse.gid);
          }
        }
      }
    ...
    if (options.fuzzy === false || !fuseInstance) {
      return {
        type: "fuzzy",
        gids: /* @__PURE__ */ new Set()
      };
    }
    const fuseResults = fuseInstance.search(token);
    const hasHighQualityMatches = fuseResults.some((res) => res.score !== void 0 && res.score <= 0.25);
    const cutoff = hasHighQualityMatches ? 0.35 : 0.5;
    const fuzzyGids = /* @__PURE__ */ new Set();
    const fuseMatchesMap = /* @__PURE__ */ new Map();
    fuseResults.filter((res) => res.score !== void 0 && res.score <= cutoff).forEach((res) => {
  ...
  const results = Array.from(intersection).map((gid) => {
    const verse = gidToVerse.get(gid);
    if (!verse) return null;
    const allFuseMatches = [];
    ...
    return __spreadProps(__spreadValues({}, verse), {
      fuseMatches: allFuseMatches.length > 0 ? [...allFuseMatches] : void 0
    });
  }).filter((verse) => verse !== null);
  return results;
};
var search2 = (query, quranData, morphologyMap, wordMap, options = {
  lemma: true,
  root: true
}, pagination = {
  page: 1,
  ...
  const fuseInstance = fuzzyEnabled ? createArabicFuseSearch(quranData, ["standard", "uthmani"]) : null;
  const simpleMatches = simpleSearch(quranData, cleanQuery, "standard");
  const advancedMatches = performAdvancedLinguisticSearch(cleanQuery, quranData, options, fuseInstance, wordMap, morphologyMap);
  const allMatches = [...simpleMatches, ...advancedMatches];
  const gidSet = /* @__PURE__ */ new Set();
  const combined = [];
  const mapEntry = wordMap[cleanQuery];
  for (const verse of allMatches) {
    if (!gidSet.has(verse.gid)) {
      gidSet.add(verse.gid);
      const fuseMatches = "fuseMatches" in verse ? verse.fuseMatches : void 0;
      combined.push(computeScore2(verse, cleanQuery, morphologyMap, wordMap, options, mapEntry, fuseMatches));
    }
  }
...
//# sourceMappingURL=quran-search-engine.js.map
